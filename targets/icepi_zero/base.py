...
Per-project top files (targets/icepi_zero/mnist.py, targets/icepi_zero/lcd.py) import from here
...
# This file lives at targets/icepi_zero/, so the repo root is three levels up.
# The tops are run as scripts (python targets/icepi_zero/all.py), which puts
# their own directory on sys.path but NOT the repo root -- so `gateware` and
# `sim` have to be added explicitly or every top fails on import.
...
class BaseSoC(SoCCore):
    ...
    def __init__(self, sys_clk_freq=50e6,
        ...
        # unused. See targets/icepi_zero/c3loader.py.
        if with_sdram:
