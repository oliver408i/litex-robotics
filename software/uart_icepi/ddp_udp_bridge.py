...
def main():
    ...
    parser.add_argument("--min-fps", type=float, default=30.0,
                        help="Warn if push-frame rate drops below this.")
    parser.add_argument("--warn-interval", type=float, default=2.0,
                        help="Minimum seconds between rate warnings.")
        ...
        while True:
            ...
            if header["flags"] & DDP_FLAG_PUSH:
                ...
                if now - fps_window_start >= 1.0:
                    ...
                    if fps < args.min_fps and (now - last_warn_ts) >= args.warn_interval:
                        print(f"Warning: DDP push rate low ({fps:.1f} fps < {args.min_fps:.1f})")
                        last_warn_ts = now
                    fps_window_start = now
                    fps_window_count = 0
                if args.verbose:
                ...
                if args.min_fps > 0 and last_push_ts != 0.0:
                    now = time.monotonic()
                    if (now - last_push_ts) > (1.5 / args.min_fps) and (now - last_warn_ts) >= args.warn_interval:
                        print("Warning: no DDP push frames seen recently")
                        last_warn_ts = now
