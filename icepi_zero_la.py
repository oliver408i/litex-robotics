...
A deployable LA bitstream: the deployable baseline (aux SPI bus + boot_ctl,
see gateware/soc_features.add_flashing_baseline) plus the SDRAM-streaming
capture core (add_logic_analyzer). The 18 probe channels are the GPIO-bank
pins that free up once the LCD/touch module is unplugged.
...
JTAG. The aux bus (IMU/MCP, cs[0] WINC line now parked) is untouched.
...
class LogicAnalyzerSoC(BaseSoC):
    def __init__(self, sys_clk_freq=50e6, with_spi_flash=False, flash_boot_offset=None,
                 flash_master=False, aux_spi_clk_freq=12.5e6, **kwargs):
        super().__init__(
        ...
        add_flashing_baseline(self, aux_spi_clk_freq=aux_spi_clk_freq, busy_led=0)
...
def main():
    ...
    parser.add_target_argument("--aux-spi-clk-freq", default=12.5e6, type=float,
                               help="Aux-bus default SCK frequency (Hz). Default 12.5 MHz (tested "
