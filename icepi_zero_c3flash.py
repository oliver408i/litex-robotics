...
XIP-safety: the loader firmware runs from main_ram (BRAM, not flash), so it never
instruction-fetches through the flash mmap while it issues master commands (the
mode-knock hazard in flash_w25q.h / docs/boot_chain.md). SDRAM is dead, so main_ram
is a 32 KB BRAM. Forces the flash-master deployment shape (with_spi_flash +
flash_master) so flash_w25q.c has its spiflash_master_* CSRs.

Bring-up: serial-boot the loader over UART (SDRAM down -> load to main_ram BRAM):
...
class C3FlashSoC(BaseSoC):
    def __init__(self, sys_clk_freq=50e6, flash_boot_offset=None, **kwargs):
        ...
        super().__init__(
            ...
            with_sdram        = False,   # SDRAM dead; main_ram is BRAM (XIP-safe fw home)
            flash_boot_offset = flash_boot_offset,
            **kwargs,
        )
        add_c3_spibone(self)     # ESP32-C3 as a Wishbone master over SPI
        add_c3_mailbox(self)     # command/data mailbox RAM @ 0x30000000
