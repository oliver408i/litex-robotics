#!/usr/bin/env python3
"""IcePi Zero deployable: the IMU data logger with LCD/touch UI (WiFi-updatable).

The field logger SoC. Composes, on the mandatory WiFi-OTA baseline:
  ...
  - the OTA-flashing baseline (add_flashing_baseline): XIP BIOS + LiteSPI
    master + ATWINC1500 aux bus + boot-manager flag/reset. The LSM6DS3 IMU
    rides this same aux bus on cs[1] FOR FREE (firmware drives it via
    aux_spi.c's AUX_IMU) -- so the logger needs no separate IMU SPI block.

It omits the SNN: on the 25F, SD's two block-DMA masters fit alongside LCD+WINC
only once the SNN (the ~78%-BRAM heavy block) is dropped. Verified to route
...
icepi_zero_mnist_lcd.py; SD is mutually exclusive with the LCD+SNN pair.) WiFi
is in because flash.py is WiFi-only -- this lets the field unit be updated in
place rather than retrieved for a JTAG reflash. No SNN means the user LEDs are
free, so the WINC busy LED stays at index 0.
...
class LoggerSoC(BaseSoC):
    def __init__(self, sys_clk_freq=50e6, flash_boot_offset=None,
        ...
        # WiFi-OTA baseline: brings the aux bus the IMU (cs[1]) rides on. No SNN
        # here, so the WINC busy LED keeps index 0.
...
def main():
    ...
    parser.add_target_argument("--winc-spi-clk-freq", default=12.5e6, type=float,
                               help="Aux-bus default SCK frequency (Hz), shared by the WINC "
                                    "and the LSM6DS3 IMU (cs[1]). Default 12.5 MHz; the bus is "
                                    "capped at sys/2 = 25 MHz and runtime-adjustable per device "
                                    "via the aux_spi clk_divider CSR (LSM6DS3 tops out at 10 MHz).")
