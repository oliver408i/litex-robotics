...
void D_DoomMain (void)
{
    ...
    // IcePi Zero: no filesystem, so skip disk-based IWAD discovery. The WAD is
    // served from flash by the memory-backed w_file backend (w_file_icepi.c);
    // hardcode the shareware DOOM identity. D_IdentifyVersion() below still
    // inspects loaded lumps to settle gamemode (shareware vs registered).
    iwadfile = "doom1.wad";
    gamemission = doom;
