#!/usr/bin/env python3
"""Assembler for the PSC18SR (PISC v2) draft encoding.

PROTOTYPE against a DRAFT ISA -- docs/psc18sr_isa_draft.md. Separate file from
...
Usage:
    python3 tools/psc18sr_asm.py prog.s --hex prog.hex     # $readmemh, for INITVAL
    python3 tools/psc18sr_asm.py prog.s --list             # address/word/source
    python3 tools/psc18sr_asm.py prog.s --py               # python list, for CSR loads
    python3 tools/psc18sr_asm.py prog.s --c sum            # C array, for firmware
...
# ALU funct field. 0x08-0x1F are reserved and deliberately unassigned; the
# assembler refuses them so a program cannot depend on decode-to-ADD.
#
# The shifts take their amount from rs2 like every other ALU op -- there is no
# shift-immediate form, because a pseudo-op here must assemble to exactly one
# real instruction and a constant amount would need an LI first. Shifting by a
# literal is therefore two instructions, written as such.
...
class Assembler:
    ...
    def encode(self, mnem: str, ops: list[str], pc: int, pass2: bool) -> int:
        ...
        if m in FUNCT:                          # ADD/SUB/AND/OR/XOR/SLL/SRL/SRA rd,rs,rs2
...
def main() -> int:
    ap = argparse.ArgumentParser(description="PSC18SR (PISC v2) assembler -- DRAFT ISA.")
    ...
    ap.add_argument("--c", metavar="NAME",
                    help="print a C fragment: the program as a uint32_t array plus "
                         "its origin, length, and every label. Firmware loads the "
                         "array at ORG and starts there; the label defines are what "
                         "let it patch a constant-pool word whose value is only "
                         "knowable at runtime (a CSR offset) without hand-counting "
                         "instructions.")
    ...
    if args.list or not (args.hex or args.py or args.c):
    ...
    if args.c:
        name = args.c
        up   = name.upper()
        org  = min(a for a, _, _ in out) if out else 0
        top  = max(a for a, _, _ in out) + 1 if out else 0
        img  = [0] * (top - org)
        for addr, word, _ in out:
            img[addr - org] = word
        print(f"/* generated by tools/psc18sr_asm.py from {args.source} -- do not edit */")
        print(f"#define PSC18SR_{up}_ORG   {org}")
        print(f"#define PSC18SR_{up}_WORDS {len(img)}")
        for label, addr in sorted(asm.labels.items(), key=lambda kv: kv[1]):
            print(f"#define PSC18SR_{up}_L_{label.upper()} {addr}")
        print(f"static const uint32_t psc18sr_{name}[] = {{")
        for i in range(0, len(img), 8):
            print("\t" + " ".join(f"0x{w:05x}," for w in img[i:i + 8]))
        print("};")
    if args.hex:
        top = max(a for a, _, _ in out) + 1 if out else 0
