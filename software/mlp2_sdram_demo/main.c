...
int main(void)
{
	static const int8_t inputs_src[16] = {
		3, -2, 1, 4, -1, 2, 0, -3, 1, 1, -2, 2, -1, 0, 3, -2
	};
	static const int8_t w1_src[8][16] = {
	...
	static const int16_t b1_src[8] = { 1, -1, 0, 2, -2, 1, 0, -1 };
	static const int8_t w2_src[4][8] = {
		{ 1, -1, 2, 0, 1, 0, -1, 1 },
		{ 0, 2, -1, 1, -2, 1, 0, -1 },
		{ 1, 0, 1, -1, 0, 1, -2, 2 },
		{ -1, 1, 0, 2, 1, -2, 1, 0 },
	};
	static const int16_t b2_src[4] = { 0, 1, -1, 2 };

	static int8_t inputs[16];
	static int8_t w1[8][16];
	static int16_t b1[8];
	static int8_t w2[4][8];
	static int16_t b2[4];

	uart_init();
	irq_setie(1);

	uart_write_str("\r\nMLP2 SDRAM demo start\r\n");
	...
	/* Copy into SDRAM-backed buffers. */
	memcpy(inputs, inputs_src, sizeof(inputs));
	memcpy(w1, w1_src, sizeof(w1));
	memcpy(b1, b1_src, sizeof(b1));
	memcpy(w2, w2_src, sizeof(w2));
	memcpy(b2, b2_src, sizeof(b2));

	mlp_accel_in_ptr_write((uint32_t)inputs);
	mlp_accel_w1_ptr_write((uint32_t)w1);
	mlp_accel_b1_ptr_write((uint32_t)b1);
	mlp_accel_w2_ptr_write((uint32_t)w2);
	mlp_accel_b2_ptr_write((uint32_t)b2);

	mlp_accel_ctrl_write(0x2); /* clear done */
	mlp_accel_ctrl_write(0x5); /* start + load_from_mem */
